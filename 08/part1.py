...
def process_unique_pair(unique_pair: tuple[int, int, int, int], box: list[list[str]], anti_nodes: set[tuple[int, int]]):
    ...
    first_anti_node = (first_antenna[0] + distance[0], first_antenna[1] + distance[1])
    if 0 <= first_anti_node[0] < X and 0 <= first_anti_node[1] < Y:
        anti_nodes.add(first_anti_node)
    second_anti_node = (second_antenna[0] - distance[0], second_antenna[1] - distance[1])
    if 0 <= second_anti_node[0] < X and 0 <= second_anti_node[1] < Y:
